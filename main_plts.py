...
plt.plot(g2,alpha2, label='CDW')
plt.plot(g1,delta1, label='SC')
plt.plot(g3,delta3, label='SC_W')
plt.plot(g3,alpha3, label='CDW_W')
plt.legend()
plt.grid(True)
...
plt.plot(g1, np.power(delta1,2), label=r'$\Delta_{SC}^2$')
plt.plot(g2, np.power(0.5*alpha2,2), label=r'$\Delta_{CDW}^2$')
plt.plot(g3, np.power(0.5*alpha3,2)+np.power(delta3,2), label=r'$\Delta_{CDW}^2 + \Delta_{SC}^2$')

plt.legend()
plt.xlabel(r'U/t')
plt.ylabel(r'$\Delta$')
plt.grid(True)
plt.title(r'Non interacting case: J=0')
plt.savefig('Plots/J=0_Delta_vs_U.svg')
plt.show()
...
J_values = [0.1,0.15,0.2,0.3,0.5,0.7,1]
...
for i in range(np.size(J_values)):
    g1, alpha1, delta1 = np.loadtxt("Data/J="+str(J_values[i])+"/dataJ="+str(J_values[i])+"_U=0.01-20_200_delta=0.dat", delimiter=',', unpack=True)
    g2, alpha2, delta2 = np.loadtxt("Data/J="+str(J_values[i])+"/dataJ="+str(J_values[i])+"_U=0.01-20_200_alpha=0.dat", delimiter=',', unpack=True)
    ax1.plot(g1, alpha1, label=r'J='+str(J_values[i]))
    ax2.plot(g2, delta2, label=r'J='+str(J_values[i]))

ax1.legend()
ax2.legend()
ax1.grid(True)
ax2.grid(True)
ax1.set_title(r'$\Delta_{CDW}$ setting $\Delta_{SC}=0$')
ax2.set_title(r"$\Delta_{SC}$ setting $\Delta_{CDW}=0$")
ax1.set_xlabel('U/t')
ax2.set_xlabel('U/t')
ax1.set_ylabel(r'$\Delta$')
#plt.savefig('Plots/Delta_vs_U_Something=0.svg')
plt.show()
...
for i in range(len(J_values)):
    ...
    scatter = axs[i].scatter(alpha, delta, c=g, cmap='viridis', alpha=0.7, label=f'J={J_values[i]}')
